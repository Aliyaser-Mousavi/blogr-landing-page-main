document.addEventListener("DOMContentLoaded", function () {
  ...
  // GitHub username
  const username = "Aliyaser-Mousavi";
  ...
  function canMakeRequest() {
    ...
    requestCount = parseInt(
      localStorage.getItem("github_request_count") || "0"
    );
  ...
  async function fetchGitHubData() {
    ...
    try {
      ...
      if (!userResponse.ok) {
        throw new Error(
          `GitHub API returned ${userResponse.status}: ${userResponse.statusText}`
        );
      }
      ...
      if (!reposResponse.ok) {
        throw new Error(
          `GitHub API returned ${reposResponse.status}: ${reposResponse.statusText}`
        );
      }
      ...
      if (cachedData && cachedStars) {
        try {
          ...
          githubDesc.textContent =
            userData.bio ||
            "Using cached data. API rate limit may be exceeded.";
        } catch (e) {
          githubDesc.textContent =
  ...
  function updateUI(userData, stars) {
    ...
    // Update Name and ID
    githubName.textContent = userData.name || "Aliyaser Mousavi";
    githubId.textContent = `${userData.login || username} · he/him`;

    // Update bio/description
    githubDesc.textContent =
      userData.bio ||
      "👨‍💻 I'm Aliyaser — a frontend developer learning to become full stack.";

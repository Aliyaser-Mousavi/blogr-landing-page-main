...
function createMobileSkillCard(skill) {
  ...
  card.className =
    "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg mb-4 overflow-hidden shadow-sm";

  // Handle projects section based on whether projects exist
  let projectsHTML;
  if (skill.projects && skill.projects.length > 0) {
    ...
    projectsHTML = `<p class="text-sm text-gray-500 dark:text-gray-400 text-left italic">No provided projects.</p>`;
  }

  card.innerHTML = `
    <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-3">
        <img src="${skill.icon}" loading="lazy" alt="${skill.name}" title="${
    skill.name
  }" class="w-6 h-6" />
      ...
      <div class="mt-2">
        ...
        }-100 text-${skill.proficiencyColor}-800 dark:bg-${
    skill.proficiencyColor
  }-900 dark:text-${skill.proficiencyColor}-300">
          ${skill.proficiency}
        </span>
      </div>
    </div>
...
function createDesktopSkillRow(skill) {
  ...
  if (skill.projects && skill.projects.length > 0) {
    ...
    projectsHTML = `<p class="text-gray-500 dark:text-gray-400 text-left italic">No provided projects.</p>`;
  }
...
function createCertificateCard(cert) {
  const card = document.createElement("div");
  card.className =
    "certificates-card flex flex-col justify-between items-center gap-3 bg-brand-primary p-4 rounded-3xl button-shadow max-w-80";
  ...
  card.innerHTML = `
    <div class="cer-picture w-full h-full">
      ...
      <img data-src="${
        cert.imagePath
      }" class="lazy opacity-0 transition-opacity duration-300" alt="${
    cert.imageAlt
  }" />
    </div>
    <div class="cer-context h-full w-full flex flex-col gap-2 justify-between">
      <div class="flex flex-col gap-2 flex-1">
        ...
        <div class="cer-logo w-20 py-2 px-2 bg-white rounded-full">
          <img src="${cert.providerLogo}" alt="${cert.provider} Logo" />
        </div>
        <a title="verify link" href="${
          cert.verifyLink
        }" class="cer-title font-mont-bold leading-5 hover:underline" target="_blank">
          ${cert.title}
        </a>
      </div>
      <div>
        <p class="cer-info text-xs">offered by <b>${cert.provider}</b> in <b>${
    cert.platform
  }</b></p>
        <a href="${cert.downloadPath}" download="${cert.downloadPath
    .split("/")
    .pop()}" class="cer-download" target="_blank">
